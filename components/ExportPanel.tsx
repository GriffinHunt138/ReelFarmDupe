...
export default function ExportPanel({ postId, outputDir, slidePaths, previews, tiktokConnected, title }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div className="text-xs text-white/40 uppercase tracking-widest font-semibold">Export</div>

      {!hasSlides && (
        <div className="text-sm text-white/30 bg-zinc-900/50 border border-white/8 rounded-xl px-4 py-3">
      ...
      <div className="flex gap-3 flex-wrap">
        <button
          ...
          className="flex items-center gap-2 bg-white text-black font-semibold text-sm px-5 py-2.5 rounded-xl hover:bg-white/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
          {zipping ? (
            <>
      ...
      {!tiktokConnected && hasSlides && (
        <div className="text-xs text-white/40">Connect your TikTok account above to enable direct posting.</div>
      )}

      {postResult?.success && (
        <div className="flex items-center gap-2 bg-green-950/50 border border-green-500/30 rounded-xl px-4 py-3 text-green-400 text-sm">
      ...
      {postResult?.error && (
        <div className="bg-red-950/50 border border-red-500/30 rounded-xl px-4 py-3 text-red-400 text-sm">
          {postResult.error}
        </div>
      )}

      {savedZip && (
        <div className="flex items-start gap-2 bg-green-950/50 border border-green-500/30 rounded-xl px-3 py-2.5 text-green-400 text-xs">
      ...
      {zipError && (
        <div className="text-red-400 text-xs">{zipError}</div>
      )}
    </div>
  );
}
